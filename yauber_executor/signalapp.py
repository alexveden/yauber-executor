...
RABBIT_EXCHANGE = "YAUBER_EXECUTOR"
...
class SignalApp(object):
    ...
    def connect(self):
        ...
        """
        log.info('Connecting to RabbitMQ host')
    ...
    def listen(self, callback):
        ...
        if callback is None or not callable(callback):
            raise ValueError("callback function is None or not callable")

        sign = inspect.getfullargspec(callback)
        if sign.args != ['appclass', 'appname', 'msg']:
            raise ValueError("callback function has invalid signature, expected callback(appclass, appname, msg) got callback{0} ".format(tuple(sign.args)))
    ...
    def on_message(self, unused_channel, method, properties, body):
        ...
        try:
            log.info('Received message # %s from %s: %s',
                        method.delivery_tag, method.routing_key, body)

            if '.' not in method.routing_key:
                log.error('Bad routing key format: {0}'.format(method.routing_key))
                return
            ...
            try:
                ...
                log.error('Failed to load message: {0}'.format(exc))
                return

            if self.listen_callback is not None:
                self.listen_callback(appclass, appname, data_object)
        except:
            log.exception('Exception while processing message')
        finally:
